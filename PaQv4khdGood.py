# -*- coding: utf-8 -*-
"""
Created on Wed Jun 21 22:29:54 2023
...
# 在循环外部，初始化序列号
image_counter = 1
def save_imgs(title, src, subfolder):
...
def scrap_images(url, folder):
    ...
    for group in groups:
        ...
        save_imgs(title, src, subfolder)  # 将子文件夹的名字传递给save_imgs函数
        a_tag = group.find('a')
        ...
        for i in range(1, num_of_li + 1):
            ...
            for group_detail in groups_detail:
                ...
                save_imgs(title, src_detail, subfolder)  # 将子文件夹的名字传递给save_imgs函数
        print("Try")
    time.sleep(0.5)
    print("OK")


folder = 'D:\\PaQv\\PaQv4khd'
